...
class FactorClustMomSeason(Factor):
    @timebudget
    ...
    def __init__(self,
        ...
        self.factor_data = pd.read_parquet(get_factor_dir(self.live) / 'factor_mom_season.parquet.brotli')
    ...
    def function(self, splice_data):
        ...
        for i, col in enumerate(splice_data.columns.get_level_values(0).unique()):
            ...
            cols = data.columns
            date = data.index[0]
            data = pd.DataFrame(cluster, columns=[f'{col.lower()}_cluster'], index=[[date] * len(cols), cols])
