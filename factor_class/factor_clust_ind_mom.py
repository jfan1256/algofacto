...
class FactorClustIndMom(Factor):
    ...
    def __init__(self,
        ...
        t = 1
        ret = create_return(combine, windows=[t])[[f'RET_{t:02}', 'ind']]
        avg_ret = ret.groupby(['ind', ret.index.get_level_values('date')])[f'RET_{t:02}'].mean()
        ret = ret.reset_index()
        ret = pd.merge(ret, avg_ret.rename('indRET').reset_index(), on=['ind', 'date'], how='left')
        ret[f'IndMom_{t:02}'] = ret[f'RET_{t:02}'] / ret['indRET']
        ind_mom = ret.set_index([self.join, 'date'])[[f'IndMom_{t:02}']]
        self.factor_data = ind_mom
        self.cluster = cluster
        # Create returns and convert stock index to columns
        self.factor_data = self.factor_data['IndMom_01'].unstack(self.join)
