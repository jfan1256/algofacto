...
class FactorClustVolatility(Factor):
    ...
    def __init__(self,
        ...
        self.factor_data[f'Volatility_01'] = self.factor_data.groupby('ticker')[f'RET_01'].rolling(window=60).std().reset_index(level=0, drop=True)
        self.factor_data = self.factor_data.drop('RET_01', axis=1)
        self.factor_data = self.factor_data[[f'Volatility_01']]
        self.factor_data = self.factor_data['Volatility_01'].unstack('ticker')
