...
class FactorIndMom(Factor):
    ...
    def __init__(self,
        ...
        # for t in T:
        #     avg_ret = ret.groupby(['Industry', 'date'])[f'RET_{t:02}'].transform('mean')
        #     ret[f'IndMom_{t:02}'] = np.where(ret[f'RET_{t:02}'] > avg_ret, 1, 0)
        #     # ret[f'IndMom_{t:02}'] = ret[f'RET_{t:02}']/avg_ret
        #     # ret[f'IndMom_{t:02}'] = ret.groupby('date')[f'IndMom_{t:02}'].rank()
        ...
        for t in T:
            ...
            avg_ret = ret.groupby(['ind', 'date'])[f'RET_{t:02}'].transform('mean')
            ret[f'IndMom_{t:02}_rank'] = np.where(ret[f'RET_{t:02}'] > avg_ret, 1, 0)
            ind_mom = ret[[f'IndMom_{t:02}', f'IndMom_{t:02}_rank']]
            collect.append(ind_mom)
