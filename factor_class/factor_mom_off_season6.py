...
class FactorMomOffSeason6(Factor):
    @timebudget
    ...
    def function(self, splice_data):
        ...
        def compute_mom(group):
            ...
            group['retTemp1'] = group[[f'temp{i}' for i in range(71 * scale_factor, 121 * scale_factor, 12 * scale_factor)]].sum(axis=1, skipna=True)
            group['retTemp2'] = group[[f'temp{i}' for i in range(71 * scale_factor, 121 * scale_factor, 12 * scale_factor)]].notna().sum(axis=1)
            ...
            group['mom_off_season6'] = (group['retLagTemp_sum60'] - group['retTemp1']) / (group['retLagTemp_count60'] - group['retTemp2'])

            return group[['mom_off_season6']]
