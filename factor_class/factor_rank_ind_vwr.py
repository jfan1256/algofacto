...
class FactorRankIndVWR(Factor):
    ...
    def __init__(self,
        ...
        ind_data['value_ind'] = ind_data.groupby(['Industry', 'date'])['value_permno'].transform('sum')
        ...
        for t in T:
            ...
            ind_data[f'rank_vwr{t:02}'] = ind_data.groupby(['Industry', 'date'])[f'vwr_{t:02}'].rank(method='dense')
            collect.append(ind_data[[f'rank_vwr{t:02}']])
