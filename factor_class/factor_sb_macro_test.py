...
class FactorSBMacroTest(Factor):
    @timebudget
    ...
    def __init__(self,
        ...
        # self.macro_data = pd.read_parquet(get_load_data_parquet_dir() / 'data_macro_test.parquet.brotli')
    ...
    def function(self, splice_data):
        ...
        t = 1
        ret = f'RET_{t:02}'
        splice_data[ret] = np.log(1+splice_data[ret])

        # if window size is too big it can create an index out of bound error (took me 3 hours to debug this error!!!)
        windows = [30, 60]
        for window in windows:
            betas = rolling_ols_sb(price=splice_data, factor_data=self.macro_data, factor_col=self.factor_col, window=window, name='MACRO_TEST', ret=ret)
