...
save = False
params = {
    'max_depth':         {'optuna': ('suggest_int', 1, 20),                     'gridsearch': [-1, 6, 10],                   'default': -1},
    'learning_rate':     {'optuna': ('suggest_float', 0.15, 0.15, False),       'gridsearch': [0.005, 0.01, 0.1, 0.15],      'default': 0.15},
    'num_leaves':        {'optuna': ('suggest_int', 5, 50),                     'gridsearch': [20, 40, 60],                  'default': 15},
    ...
    'lambda_l1':         {'optuna': ('suggest_float', 1e-5, 1, False),          'gridsearch': [0.001, 0.01],                 'default': 0},
...
alpha = AlphaModel(model_name='lightgbm_trial_30', tuning=['optuna', 30], plot_loss=False, plot_hist=False, pred='price',
                   lookahead=1, incr=True, pretrain_len=1260, train_len=504, valid_len=21, test_len=21, **params)
...
# rank_fund_ratio = PrepFactor(factor_name='factor_rank_fund_ratio', interval='M', kind='fundamental', tickers='all', div=False, start=start, end=end, save=save).prep()
# alpha.add_factor(rank_fund_ratio)
# del rank_fund_ratio

# clust_fund_ratio = PrepFactor(factor_name='factor_clust_fund_ratio', interval='M', kind='cluster', tickers='all', div=False, start=start, end=end, save=save).prep()
# alpha.add_factor(clust_fund_ratio, categorical=True)
# del clust_fund_ratio

# fund_ratio = PrepFactor(factor_name='factor_fund_ratio', interval='M', kind='fundamental', tickers='all', div=False, start=start, end=end, save=save).prep()
# alpha.add_factor(fund_ratio)
# del fund_ratio
#
# high = PrepFactor(factor_name='factor_high', interval='D', kind='price', div=False, tickers=tickers, start=start, end=end, save=True).prep()
# alpha.add_factor(high)
# del high
#
# low = PrepFactor(factor_name='factor_low', interval='D', kind='price', div=False, tickers=tickers, start=start, end=end, save=True).prep()
