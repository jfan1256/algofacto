...
class ModelBayes(ModelTrain):
    ...
    def lregression(self, export_key, param_name_train, param_val_train, base_params, data_train, ret, factors, metric_cols):
        # Get the parameters
        ...
        for i, (train_idx, test_idx) in enumerate(opt):
            ...
            if self.valid_len != 0:
                br_train = data_train.iloc[train_idx[:-self.valid_len]]
                br_val = data_train.iloc[train_idx[-self.valid_len:]]

                # Split into Train/Validation
                X_train = br_train.drop(ret, axis=1)
                y_train = br_train[ret]
                X_val = br_val.drop(ret, axis=1)
                y_val = br_val[ret]
            else:
                br_train = data_train.iloc[train_idx]
            ...
            if self.pred == 'price' and self.model == 'bayesian_ridge':
    ...
    def train_option(self, trial):
        ...
        if self.tuning[0] == 'optuna':
            ...
            return self.lregression(key, param_names, param_vals, base_params, data_train, ret, factors, metric_cols)
            ...
            for p, param_vals in enumerate(cv_params_):
                ...
                os.makedirs(get_ml_result(self.live, self.model_name) / f'{self.model_name}' / f'params_{key}')
                # Train model
                self.lregression(key, param_names, param_vals, base_params, data_train, ret, factors, metric_cols)
